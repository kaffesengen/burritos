...
function update() {
    ...
    if (isHost) {
        ...
        for (let pid of pkeys) {
            ...
            let shiftDownRpm = isGokart ? 6500 : 3500;
            ...
            if (raceState > 0) { 
                ...
                if (preset.ev) {
                    ...
                    if (!p.manualGear) {
                        if (p.rpm > shiftUpRpm && p.gear < 5 && p.shiftTimer <= 0) { p.gear++; p.shiftTimer = shiftDelay; } 
                    ...
                    if ((mThrottle > 0 || isRev) && p.gear > 0 && p.shiftTimer <= 0) {
                        ...
                        if (isGokart) {
                            let tqMul = 0;
                            if (p.rpm < 4000) tqMul = 0.12;
                            else if (p.rpm < 7000) tqMul = 0.12 + ((p.rpm - 4000) / 3000) * 0.15;
                            else if (p.rpm < 11000) tqMul = 0.27 + ((p.rpm - 7000) / 4000) * 0.73;
                            else if (p.rpm < 13000) tqMul = 1.0 - ((p.rpm - 11000) / 2000) * 0.64;
                            else tqMul = 0.1;
                            torqueVal = aPower * 2.5 * tqMul;
                        } else {
