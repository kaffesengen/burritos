const vehiclePresets = {
    ...
    gokart: { power: 45, mass: 180, drivetrain: 'RWD', grip: 2.80, turn: 5.5, roll: 0.02, w: 14, l: 24, ev: false, type: 'gokart', fuelCap: 10, audio: 'gokart' },
...
function update() {
    ...
    if (isHost) {
        ...
        for (let pid of pkeys) {
            ...
            let maxEngineRpm = isGokart ? 13500 : 7500;
            let shiftUpRpm = isGokart ? 12800 : 7200;
            let shiftDownRpm = isGokart ? 6500 : 3500;
            let shiftDelay = isGokart ? 0.02 : 0.15;
            let aeroDrag = isGokart ? 0.65 : 0.45;
            
            let kartGearRatios = [0, 2.14, 1.71, 1.44, 1.24, 1.09, 0.96];
            let currentGearRatios = isGokart ? kartGearRatios : gearRatios;
            let currentFinalDrive = isGokart ? 4.95 : finalDrive; 
            let currentWheelRadius = isGokart ? 0.14 : wheelRadius; 
            let maxGears = isGokart ? 6 : 5;
            ...
            if (raceState > 0) { 
                ...
                if (preset.ev) {
                    ...
                    if (p.rpm > maxEngineRpm + 500) p.rpm = maxEngineRpm + 500;
                    ...
                    if ((mThrottle > 0 || isRev) && p.gear > 0 && p.shiftTimer <= 0) {
                        ...
                        dForce = (torqueVal * currentGearRatios[p.gear] * currentFinalDrive / currentWheelRadius) * mThrottle;
