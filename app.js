const vehiclePresets = {
    jaguar: { power: 400, mass: 2200, drivetrain: 'AWD', grip: 6.0, turn: 4.0, roll: 0.05, w: 24, l: 52, ev: true, type: 'jaguar', fuelCap: 100 },
    gokart: { power: 30, mass: 150, drivetrain: 'RWD', grip: 3.5, turn: 5.5, roll: 0.02, w: 16, l: 26, ev: false, type: 'gokart', fuelCap: 10 },
    mx5: { power: 184, mass: 1050, drivetrain: 'RWD', grip: 3.6, turn: 4.5, roll: 0.04, w: 20, l: 42, ev: false, type: 'mx5', fuelCap: 50 },
    r34: { power: 330, mass: 1560, drivetrain: 'AWD', grip: 4.1, turn: 3.8, roll: 0.04, w: 22, l: 48, ev: false, type: 'r34', fuelCap: 70 },
    s15: { power: 450, mass: 1240, drivetrain: 'RWD', grip: 1.8, turn: 5.5, roll: 0.04, w: 21, l: 45, ev: false, type: 's15', fuelCap: 65 },
    f1: { power: 1050, mass: 798, drivetrain: 'RWD', grip: 8.0, turn: 6.0, roll: 0.09, w: 26, l: 64, ev: false, type: 'f1', fuelCap: 110 }
};
...
let raceState = -1; // -1: Free, 1-5: Lights, 0: Go
...
document.getElementById('btn-enter-game').addEventListener('click', () => { Object.values(connections).forEach(c => c.send({ type: 'start' })); enterGame(); });

// Runtime Host Adjustments
...
function initJoiner(hostId) {
    ...
    peer.on('open', id => {
        ...
        hostConnection.on('data', data => {
            if (data.type === 'init') { activeTrackId = data.trackId; activeTrack = tracks[activeTrackId]; generateEnvironment(); } 
...
function update() {
    ...
    if (isHost) {
        ...
        for (let pid of pkeys) {
            ...
            if (p.fuel <= 0) { p.fuel = 0; ins.throttle = 0; } // Engine dead

            // Surface
            ctx.lineWidth = 160; const onAsphalt = ctx.isPointInStroke(activeTrack.path, p.x, p.y);
            ctx.lineWidth = 240; const onSand = ctx.isPointInStroke(activeTrack.path, p.x, p.y) && !onAsphalt;
            ctx.lineWidth = 250; const inBounds = ctx.isPointInStroke(activeTrack.path, p.x, p.y);
            let sMu = onAsphalt ? preset.grip : (onSand ? preset.grip*0.4 : preset.grip*0.55);

            if (!inBounds && dt > 0) {
                p.x = p.prevX; p.y = p.prevY; p.vx *= 0.5; p.vy *= 0.5;
                p.x += -Math.sign(p.vx)*5; p.y += -Math.sign(p.vy)*5; // Crude bounce bounds
            }

            // Lock controls during red lights
            if (raceState > 0) { ins.throttle = 0; ins.handbrake = true; }

            p.steer += ((Math.abs(ins.steering)>0.08?Math.sign(ins.steering)*((Math.abs(ins.steering)-0.08)/0.92):0) - p.steer) * 12.0 * dt;
            ...
            p.rpm += (((preset.ev ? p.speedKmh*80 : (p.speedKmh<1 ? 1000+(Math.max(0,mThrottle)*6500) : 3000))) - p.rpm) * 12 * dt;
            p.gear = preset.ev ? (lVx >= -0.5 ? 'D' : 'R') : 1;

            const maxLat = (preset.mass * 9.81 / 2) * sMu * 1.40; 
            let dF = (mThrottle > 0 || (mThrottle<0&&lVx<1.0)) ? (preset.power*735*0.85/Math.max(Math.abs(lVx),5.0))*mThrottle : 0;
            let bF = (mThrottle<0&&lVx>=1.0) ? preset.mass*15.0*Math.abs(mThrottle) : 0;
            let fLong = Math.max(-maxLat, Math.min(maxLat, dF - bF));
            let gLat = maxLat * (ins.handbrake ? 0.2 : Math.sqrt(Math.max(0.01, 1.0 - Math.pow(Math.abs(fLong)/maxLat, 2))));
            
            let delta = Math.max(-0.6, Math.min(0.6, p.steer * 0.6 - (p.speedKmh>10?Math.atan2(lVy, Math.abs(lVx)):0)*0.5));
            let slipF = lVy + p.yawRate*(preset.l/24) - lVx*delta; let slipR = lVy - p.yawRate*(preset.l/24);
            
            let fLatF = Math.max(-gLat, Math.min(gLat, -slipF * preset.mass));
            let fLatR = Math.max(-gLat, Math.min(gLat, -slipR * preset.mass));
            
            lVx += ((fLong - fLatF*Math.sin(delta) - 0.45*lVx*Math.abs(lVx))/preset.mass) * dt;
            lVy += ((fLatF*Math.cos(delta) + fLatR - 0.45*lVy*Math.abs(lVy))/preset.mass) * dt;
            p.yawRate += ((fLatF*Math.cos(delta)*(preset.l/24) - fLatR*(preset.l/24) - 2.0*p.yawRate*(preset.mass*preset.w*preset.l/12))/preset.mass) * dt;

            p.vx = lVx * Math.cos(p.angle) - lVy * Math.sin(p.angle); p.vy = lVx * Math.sin(p.angle) + lVy * Math.cos(p.angle); p.angle += p.yawRate * dt;
            p.prevX = p.x; p.prevY = p.y; p.x += p.vx * 12 * dt; p.y += p.vy * 12 * dt;

            p.frontSpinSeverity = Math.abs(slipF)>5 ? 1:0; p.rearSpinSeverity = Math.abs(slipR)>5 ? 1:0;
            outState.players[pid] = { x: p.x, y: p.y, a: p.angle, s: p.steer, fS: p.frontSpinSeverity, rS: p.rearSpinSeverity, presetId: p.presetId, g: p.gear, rpm: p.rpm, v: p.speedKmh, f: p.fuel };
        }

        // Kollisjoner (AABB Sirkel)
        for(let i=0; i<pkeys.length; i++) {
            for(let j=i+1; j<pkeys.length; j++) {
                ...
                if(dist < rA + rB && dist > 0) {
                    ...
                    let corr = (rA+rB - dist) / (1/mA + 1/mB) * 0.8;
    ...
    if (players[myId]) {
        ...
        document.getElementById('hud-gear').innerText = me.gear;
    ...
    for (let pid in players) {
        ...
        let hl=pre.l/2, hw=pre.w/2, d=Math.max(-0.6, Math.min(0.6, p.steer*0.6));
        
        ctx.fillStyle='#111'; 
        ctx.fillRect(-hl+4, -hw-1, 14, 6); ctx.fillRect(-hl+4, hw-5, 14, 6);
        ctx.save(); ctx.translate(hl-6, -hw+2); ctx.rotate(d); ctx.fillRect(-7,-3,14,6); ctx.restore();
        ctx.save(); ctx.translate(hl-6, hw-2); ctx.rotate(d); ctx.fillRect(-7,-3,14,6); ctx.restore();

        if (pre.type === 'f1') { ctx.fillStyle='#e74c3c'; ctx.fillRect(-hl+6,-hw*0.4,pre.l-12,pre.w*0.8); } 
        else if (pre.type === 'gokart') { ctx.fillStyle='#7f8c8d'; ctx.fillRect(-hl+2,-hw*0.5,pre.l-4,pre.w); } 
        else { ctx.fillStyle='#2980b9'; ctx.fillRect(-hl,-hw,pre.l,pre.w); }
